if Code.ensure_loaded?(Exgit) do
  defmodule JustBash.FS.GitFS do
    ...
    `GitFS` is **optional**: `just_bash` compiles and runs without `exgit`. The
    module itself is only defined when `exgit` is loadable at compile time, so
    `JustBash.FS.GitFS` will not exist at all in an app that doesn't declare
    the dep — a clean `UndefinedFunctionError` at the call site tells users
    ...
    # An unparseable mode indicates an exgit bug or a malformed tree — crash
    # rather than silently pretending it's a regular file. The error bubbles
    # up to the caller with a descriptive message.
    defp parse_mode(mode_str) do
      case Integer.parse(mode_str, 8) do
        ...
        _ ->
          raise ArgumentError,
                "JustBash.FS.GitFS: exgit returned an unparseable mode string: " <>
                  inspect(mode_str)
      end
    end
